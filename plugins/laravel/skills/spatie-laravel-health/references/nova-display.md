...
- **Nova Resource 类名必须与模型名一致**: `HealthCheckResultHistoryItem`, 这样 Nova 才能自动发现. 禁止用 `HealthCheckResult` 等自定义名称.
...
- 字段使用 `Panel` 分组展示(检查信息/时间信息/性能数据), 禁止平铺
...
- Resource 内通过 `authorizedToCreate/Update/Delete/Replicate` 返回 `false` 实现只读, 无需额外 Policy
