...
```bash
# 逐个检查工具可用性
slither --version 2>/dev/null && echo "SLITHER_OK" || echo "SLITHER_MISSING"
myth version 2>/dev/null && echo "MYTHRIL_OK" || echo "MYTHRIL_MISSING"
echidna --version 2>/dev/null && echo "ECHIDNA_OK" || echo "ECHIDNA_MISSING"
halmos --version 2>/dev/null && echo "HALMOS_OK" || echo "HALMOS_MISSING"
forge --version 2>/dev/null && echo "FORGE_OK" || echo "FORGE_MISSING"
```
