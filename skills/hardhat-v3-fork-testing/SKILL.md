...
```solidity
/**
 * @title 顶层账号投资 Fork 测试
 * @notice 在 BSC 主网 Fork 环境下测试顶级账号投资功能
 *
 * 运行命令:
...
# 运行单个 Fork 测试文件
bunx hardhat test solidity contracts/TopAccountForkTest.t.sol

# 运行指定测试函数
bunx hardhat test solidity contracts/TopAccountForkTest.t.sol --grep "test_DirectTransfer"
...
- `contracts/TopAccountForkTest.t.sol` - 顶层账号投资 Fork 测试
- `contracts/TopAccountBindForkTest.t.sol` - 顶层账号绑定推荐关系 Fork 测试
- `contracts/TopAccountDirectTransferForkTest.t.sol` - 顶层账号直接转账 Fork 测试
